...
def fit_dataset():
    ...
    channs    = db.DataSiPM(db_file, run_no).SensorID.values[192:]
    ...
    for ich, (led, dar) in enumerate(zip(lspecs[192:], dspecs[192:])):
        ...
        try:
            ...
            plt.title(f'Spe response fit to channel {channs[ich]}')
            plt.xlabel('ADC')
            plt.ylabel('AU')
            plt.xlim(-30, 250)
            plt.show()
    ...
    pos_x  = db.DataSiPM(db_file, run_no).X       .values[192:]
    pos_y  = db.DataSiPM(db_file, run_no).Y       .values[192:]
    channs = db.DataSiPM(db_file, run_no).SensorID.values[192:]
