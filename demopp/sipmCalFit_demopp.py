...
parser.add_argument('--func-name',         type=str,  help='function that will be used to fit',  required=False, default='dfunc')
parser.add_argument('--use-db-gain-seeds', type=bool, help='option to take gain values from db', required=False, default=False)
parser.add_argument('--min-stat',          type=int,  help='min statistics for the peaks',       required=False, default=10)
parser.parse_args()
...
def fit_dataset(file_name=None, func_name=None, minStat=None):
    ...
    if use_db_gain_seeds:
        dodgy     = DB.DataSiPM(db_file, run_no).index[DB.DataSiPM(db_file, run_no).Active==0].values
    ...
    for ich, (led, dar) in enumerate(zip(specsL, specsD)):
        ...
        seeds, bounds = seeds_and_bounds(sens_type, run_no, ich, scaler_func, bins[b1:b2],
                                         led[b1:b2], ped_vals, 'demopp', gfitRes.errors, func_name, use_db_gain_seeds)
        if seeds[2] == 0:
            ## Channel was bad but maybe recovered
            seeds, bounds = seeds_and_bounds(sens_type, run_no, ich, scaler_func, bins[b1:b2],
                                             led[b1:b2], ped_vals, 'demopp', gfitRes.errors, func_name, use_db_gain_seeds=False)
        ...
        if not optimise:
            if channs[ich] in specialCheck or chi >= 10 or rfit.values[2] < 12 or rfit.values[2] > 19 or rfit.values[3] > 3:
                ...
                plt.xlabel('ADC')
                plt.ylabel('AU')
                #plt.savefig("/home/ryan/Code/Calib/temp2/FITsipm_number"+str(channs[ich]))
                plt.show()
    ...
    plt.scatter(pos_x, pos_y, c=pVals[3], s=300)
    ...
    plt.scatter(pos_x, pos_y, c=pVals[2], s=300)
    plt.title("Fit poisson mu")
    plt.xlabel("X (mm)")
    plt.ylabel("Y (mm)")
    plt.colorbar()
    plt.show()
    plt.scatter(pos_x, pos_y, c=pVals[0], s=300)
